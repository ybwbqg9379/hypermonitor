...
// Load ALL .env.local variables into process.env so SSR server modules
// (server/**/*.ts) can access them. Vite only populates import.meta.env
// with VITE_* prefixed vars by default — this ensures API keys like
// MASSIVE_API_KEY, FINNHUB_API_KEY, etc. are available in server handlers.
